...
export default function StepsChart({
  ...
  mode = "day",
  barColor = "#D9D9D9"
}: StepsChartProps): React.ReactElement {
    ...
    switch (mode) {
      case "day":
        return Array.from({ length: 24 }, (_, hour) => {
          const point = data.find((d) => d.x === hour);
          return { label: `${hour}h`, y: point?.y ?? 0 };
        });
      case "week":
  ...
  const drawableWidth = chartWidth - yAxisLabelWidth - (containerPaddingHorizontal * 2);
  ...
  const spacing = grouped.length > 1 ? remainingSpace / (grouped.length + 1) : 0;

  // Define rótulo com pausa
  ...
  return (
    <View style={styles.chartContainer}>
      <Svg width={chartWidth} height={chartHeight + xAxisLabelHeight + baseLineOffset + 10}>
        {grouped.map((point, index) => {
          const rawHeight = (point.y / maxY) * chartHeight;
          const minBarHeight = 10;
          const barHeight = point.y > 0 ? Math.max(rawHeight, minBarHeight) : minBarHeight;
        ...
        {grouped.map((point, index) => {
          ...
          const xPos = yAxisLabelWidth + (index * (barWidth + spacing)) + spacing + barWidth / 2;
          return (
            <SvgText
              key={`label-x-${index}`}
              x={xPos}
        ...
        <G>
          {yAxisLabels.map((tickValue, index) => {
            const yPos = chartHeight + baseLineOffset - (tickValue * yAxisScaleFactor);
            return (
              <SvgText
                key={`label-y-${index}`}
