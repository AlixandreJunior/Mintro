...
class DiaryTests(APITestCase, UserMixin):
    ...
    def test_get_diary_object(self):
        url = reverse("diary:diary_object", args=[self.diary.title])
    ...
    def test_get_diary_object_fail_for_404(self):
        url = reverse("diary:diary_object", args=["TESTE"])
    ...
    def test_patch_diary_update(self):
        url = reverse("diary:diary_update", args=[self.diary.title])
    ...
    def test_patch_diary_update_fail_for_404(self):
        url = reverse("diary:diary_update", args=["Titulo errado."])
    ...
    def test_post_diary_create_fail_for_blank(self):
        ...
        self.assertEqual(
            response.json().get("content")[0], "Este campo não pode ser em branco."
        )
    ...
    def test_post_diary_create_fail_for_duplicate(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(
            "Você já criou um diário com esse título.", response.json().get("title")
        )
